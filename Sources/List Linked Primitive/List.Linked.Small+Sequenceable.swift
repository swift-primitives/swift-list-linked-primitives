...
// module per [MOD-036] refined-C; snapshots the inline/spilled storage (avoids pointer-escape
// from the `@_rawLayout` inline bytes under the consuming yield).

extension List.Linked.Small where Element: Copyable {

    /// A single-pass consuming iterator over a snapshot of the small list's elements.
