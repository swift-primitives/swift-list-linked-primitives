...
//
// `List.Linked.Small` is unconditionally `~Copyable` (inline-plus-spill `@_rawLayout` storage):
// it cannot back a `Copyable` stdlib iterator and a pointer cursor cannot survive the
// `consuming` `Sequenceable.makeIterator()`. So — like dict-ordered's `Small` — the scalar
// iterator SNAPSHOTS the elements (via the buffer's `~Copyable`-safe `forEach` node-walk) into
// an owned `[Element]` and walks that. Snapshot is gated on `Element: Copyable`. The list still
...
extension List.Linked.Small where Element: Copyable {
    /// A single-pass scalar iterator over a snapshot of the small list's elements, front to back.
    ///
    /// Built by node-walking the inline/spilled buffer into an owned array (avoids pointer-escape
    ...
    /// Builds an owned snapshot of the inline/spilled storage by node-walking the buffer.
    @inlinable
