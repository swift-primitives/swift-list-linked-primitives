...
extension List.Linked.Bounded where Element: ~Copyable {
    ...
    @inlinable
    public var peek: Property<Peek, Self>.Borrow.Typed<Element>.Valued<N> {
        _read {
            yield Property<Peek, Self>.Borrow.Typed<Element>.Valued<N>(self)
        }
    }
}
...
where
    Tag == List<Element>.Linked<n>.Bounded.Peek,
    ...
    Element: ~Copyable
{
    ...
    public func first<R>(
        ...
        base.value._buffer.peekFront(body)
    }
    ...
    public func last<R>(
        ...
        base.value._buffer.peekBack(body)
    }
}
...
extension List.Linked.Bounded where Element: ~Copyable {
    ...
    @inlinable
    public var reversed: Property<Reversed, Self>.Borrow.Typed<Element>.Valued<N> {
        _read {
            precondition(N >= 2, "reversed requires N >= 2 (doubly-linked)")
            yield Property<Reversed, Self>.Borrow.Typed<Element>.Valued<N>(self)
        }
    }
}
...
extension Property.Borrow.Typed.Valued
where
    Tag == List<Element>.Linked<n>.Bounded.Reversed,
    Base == List<Element>.Linked<n>.Bounded,
    Element: ~Copyable
{
    ...
    public func forEach(
        ...
        base.value._buffer.forEachReversed(body)
    }
}

// Note: iteration is via the institute `Iterable` + `Sequenceable` attachables (see the
// type module's List.Linked.Bounded+Iterable.swift / +Sequenceable.swift and the scalar
// node-walk `Iterator`). The per-type `Swift.Sequence` conformance is dropped to match the
// exemplar.
