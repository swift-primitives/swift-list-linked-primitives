...
// member in the type module (List.Linked.Small+Sequenceable.swift) per [MOD-036] refined-C;
// this conformance is thin and splits the `Iterator` associated-type binding from `Iterable`'s
// via `@_implements`. `List.Linked.Small` is unconditionally `~Copyable`, so it never conformed
// to `Swift.Sequence`; it now exposes span-primitive iteration via Iterable + Sequenceable.

extension List.Linked.Small: Sequenceable where Element: Copyable {
